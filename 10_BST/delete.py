...
class BinarySearchTree:
    ...
    def __delete_node(self,value,CN):
        if CN is None:
            return None
        if value<CN.value:
            CN.left= self.__delete_node(value,CN.left)
        elif value>CN.value:
            CN.right = self.__delete_node(value,CN.right)
        else:
            if CN.right is None and CN.left is None :
                return None
            elif CN.left is None:
                CN= CN.right
            elif CN.right is None:
                CN= CN.left
            else:
                m=self.min_value(CN.right)
                CN.value=m 
                CN.right=self.__delete_node(m,CN.right)
        return CN
    def delete_node(self,value):

        self.__delete_node(value,self.root)
        return
